...
Everytime you hit the 'Run code' button, it will start over from scratch.
...
Some exercises (like this one) also contains `💡 Hints` which you can click if you are stuck.
...
to graphical elements of the plot (e.g. the x position of the scratter points).
...
You can also try adding another aesthetic, for example `fill=v43_nl` or `size=v01_pop`.
...
Of course, R is also (and even at it's core) a statistical toolkit. 
...
So, BBB's vote share can be predicted from population density (the denser the muncipality, the fewer votes),
but not by the percentage of people over 65.

**Exercise**: Change the code above predict the VVD's vote share from `c65_plus`, `v43_nl`, and `v132_income`. 
