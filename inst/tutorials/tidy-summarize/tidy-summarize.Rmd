...
For this reason, the process in R consists of two steps: grouping the data and summarize the data.
...
mutate a new value, and summarize some other values. 
...
(first argument) of the second function. To see this in action, the following snippet can be rewritten as a pipeline like this:
...
Let's look at how we can we summarize data in R. As mentioned before we first have to group the data and next we can summarize the data.

### Data

First, let's fire up tidyverse and load the gun polls data used in the earlier example. Moreover we select only four columns
...
In that case, the summary is computed per group, but instead of replacing the data frame with only the summarized data, the new column is added to the existing data frame, with the values repeated within a group:
...
For example, the code below computes the average by question and by population category.
...
# Groups:   Question [8]
 ...
 [...]
```
As you can see, the result is a data set with one row per unique group, that is, the question - population combination.
...
```{r pollsdata2, echo=FALSE}
library(tidyverse)
url <- "https://raw.githubusercontent.com/fivethirtyeight/data/master/poll-quiz-guns/guns-polls.csv"
polls <- read_csv(url) |>
 select(Question, Pollster,Population,Support,
        Rep=`Republican Support`, Rem=`Democratic Support`) 
polls
```

```{r group-multiple-ex, exercise=TRUE, exercise.setup='pollsdata2', eval=FALSE}
polls |> 
...
```{r group-multiple-ex-code-check}
grade_code()
```
