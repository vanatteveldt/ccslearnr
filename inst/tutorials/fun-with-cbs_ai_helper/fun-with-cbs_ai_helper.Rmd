...
The code in this tutorial will walk you through an analysis of this data, but there is little explanation of what is happening.
Instead, we want you to ask questions to the AI helper that is embedded on every page.
Look through the code, run and explore it and whenever something is unclear, just ask a question!

Try it out below by asking what an R function is:

```{r, echo=FALSE}
knitr::include_app("https://vanatteveldt.shinyapps.io/ai_assistant_app/", height = "450px")
```
...
Although this can be hard to master, there are many useful resources to learn more (and you can always ask your AI assistant) and it can be very rewarding (and fun!) to make nice visualizations. 
...
Have a quick look at the code above.
Do you understand what it does and how to read the scatter plot?
If not, just ask your assistant!

*Your turn*: What happens when you replace variables in the mapping with other variables, e.g. `v43_nl` or `v132_income`?
...
It may look scary at first, but if you look at the commands individually they make more sense than you would first think.
...
Take a look at the code below (and ask your assistant to explain every line you don't get):
...
In the code above, almost all lines are optional.
What happens if you remove e.g. the theme or guide information?
...
Now, we can plot this map with `geom_sg`.
...
Is the code above clear to you?
...
For this end, we need to `join` the spatial and demographic information, and then we can use other columns from the combined data set to color the regions.
For example, the code below plots the population density per municipality:
